...
function delete_repeted_files($file_dst_path, $file_dst_name_body, $file_dst_name_ext){
    if ($file_dst_name_ext=="jpg")
        $ext=array("png","gif","pdf");
    if ($file_dst_name_ext=="gif")
        $ext=array("png","jpg","pdf");
    if ($file_dst_name_ext=="png")
        $ext=array("jpg","gif","pdf");
    if ($file_dst_name_ext=="pdf")
        $ext=array("png","gif","jpgf");
        
    $fileArray = array(
        "{$file_dst_path}/{$file_dst_name_body}.$ext[0]",
        "{$file_dst_path}/{$file_dst_name_body}.$ext[1]",
        "{$file_dst_path}/{$file_dst_name_body}.$ext[2]"
    );
...
if ((isset($_POST['action']) ? $_POST['action'] : (isset($_GET['action']) ? $_GET['action'] : '')) == 'simple') {
    ...
    // ---------- XMLHttpRequest UPLOAD ---------- This is the method used (NM).
